...
int main() {
	...
	part[2] = n % 10000;
	bool zero = false; //是否在⾮非零数字前输出合适的ling
	int printCnt = 0; //⽤用于维护单词前没有空格，之后输⼊入的单词都在前⾯面加⼀一个空格。
	for (int i = 0; i < 3; i++) {
		int temp = part[i]; //三个部分，每部分内部的命名规则都⼀一样，都是X千X百X⼗十X
		for (int j = 3; j >= 0; j--) {
			int curPos = 8 - i * 4 + j; //当前数字的位置
			if (curPos >= 9) continue; //最多九位数
			int cur = (temp / J[j]) % 10;//取出当前数字，这步非常好
			if (cur != 0) {
				if (zero) {
		...
		if (i != 2 && part[i]>0) cout << ' ' << c[i + 4]; //处理理完每部分之后，最后输出单位，Yi/Wan
	}
	return 0;
}
